...
+ (void)saveInputText:(NSString *)inputText PDFAnnotation:(MyPDFAnnotation *)pdfAnnotation toFolder:(NSString *)folderName {
    ...
    NSMutableArray *textArray = [appDelegate.filePersistence loadMutableArrayFromFile:fileName inDocumentWithDirectory:fileDirectory];
    if (!textArray) {
    ...
    // 3.将数组写回文件中
    [appDelegate.filePersistence saveMutableArray:textArray toFile:fileName inDocumentWithDirectory:fileDirectory];
